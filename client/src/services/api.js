...
const BASE_URL =
  (import.meta.env.VITE_API_BASE_URL ||
    'https://taxi-booking-ilns.onrender.com') + '/api'; // ✅ parenthesis closed + /api prefix
...
export async function adminLogin(credentials) {
  const res = await apiRequest('/auth/login', { // ✅ fixed: was /admin/login
    method: 'POST',
...
export async function getFareEstimate({ pickup, drop }) {
  ...
  const [vehicles, response] = await Promise.all([
    getVehicles(),
    apiRequest(`/bookings/estimate?distance_km=${distance_km}`),
  ]);
